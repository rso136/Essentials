<?php

include 'header.php';

?>

<div class="jumbotron">
	<h1 id="jumboHeading">ESSENTIALS</h1>
	<h2 id="jumboSub">Home & Lifestyle</h2>
	<!-- <button type="button" class="btn btn-jumbo btn-lg">Join Today <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button> -->
</div>
<div class="container-fluid">
	<div class="col-md-12 col-xs-12" id="signUpFailBox">
		<h4 class='errorMsg'>Account associated with that username already exits. Please choose a different username.</h4>
		<a href='signup.php'><h4><span class="glyphicon glyphicon-log-in" aria-hidden="true"></span> Sign Up</h4></a>
	</div>
<div class="row">
    <div id="footer" class="col-md-12 col-xs-12">
    	<br>
        <p>Created by Richard Oh</p>
        <p><b>© Richard Oh</b></p>
    </div>
</div>
</div>
</body>
</html>